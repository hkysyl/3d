...
def upload(request):
    ...
    #Linux
    # loca_dir=os.listdir('~/sse-images/')
#Windoe
    loca_dir=os.listdir('static/pro/')
    print(loca_dir)
    ...
    for i in dir_files:
        ...
        if not new_i.endswith('.pcd'):
            ...
            return JsonResponse(data)
        if new_i in loca_dir:
            data={
                'code':400400,
                'msg':f'文件{i}本地文件和上传文件名冲突',
                'token':token
            }
            return JsonResponse(data)
    ...
    #     os.mkdir('~/sse-images')
    if not os.path.exists('static/pro'):
        os.mkdir('static/pro')
    try:
        upload_img(dir_files=dir_files)
    except BaseException as error:
        print(error)
        data = {
            'code': 400400,
